...
TEST(test_basicos, tabla_alumnos) {
  ...
  Registro gerva2 = Registro(campos_alumnos, {datoNat(124), datoNat(45), datoStr("Gerva"), datoStr("10001")});
...
TEST_F(DBAlumnos, validar_registro) {
  ...
  EXPECT_FALSE(db.registroValido(Registro({"LU_N", "LU_A"}, {tipoNat, tipoNat}), "libretas"));
  EXPECT_FALSE(db.registroValido(Registro({"LU", "LU_N"}, {tipoNat, tipoNat}), "libretas"));
  EXPECT_FALSE(db.registroValido(Registro({"LU", "Nombre"}, {tipoStr, tipoStr}), "alumnos"));
  EXPECT_FALSE(db.registroValido(Registro({"Editor", "Nombre"}, {tipoStr, tipoStr}), "alumnos"));

  // Registro inválido por distintos tipos
  EXPECT_FALSE(db.registroValido(Registro({"LU_N", "LU_A", "LU"}, {tipoNat, tipoStr, tipoStr}), "libretas"));
  EXPECT_FALSE(db.registroValido(Registro({"LU", "LU_A", "LU_N"}, {tipoNat, tipoNat, tipoNat}), "libretas"));
  EXPECT_FALSE(db.registroValido(Registro({"LU", "Nombre", "Editor", "OS"}, {tipoStr, tipoStr, tipoNat, tipoStr}), "alumnos"));

  // Registro invalido por claves
  EXPECT_FALSE(db.registroValido(Registro({"LU_N", "LU_A", "LU"}, {datoNat(1), datoNat(90), datoStr("1/90")}), "libretas"));
  EXPECT_FALSE(db.registroValido(Registro({"LU", "LU_N", "LU_A"},  {datoStr("1/90"), datoNat(1), datoNat(90)}),  "libretas"));
  EXPECT_FALSE(db.registroValido( Registro({"LU", "Nombre", "Editor", "OS"}, {datoStr("1/90"), datoStr("March-Orga2"), datoStr("Nano"), datoStr("Win")}), "alumnos"));

  // Registro válido
  // Clave múltiple, 1 dif
  EXPECT_TRUE(db.registroValido( Registro({"LU_N", "LU_A", "LU"}, {datoNat(1), datoNat(90), datoStr("??")}),  "libretas"));
  EXPECT_TRUE(db.registroValido( Registro({"LU", "Materia"}, {datoStr("1/90"), datoStr("Orga2")}), "materias"));

  // Clave múltiple, 1 dif, otro orden
  EXPECT_TRUE(db.registroValido(Registro({"LU_N", "LU", "LU_A"}, {datoNat(1), datoStr("??"), datoNat(90)}), "libretas"));
  EXPECT_TRUE(db.registroValido(Registro({"Materia", "LU"}, {datoStr("Orga2"), datoStr("1/90")}), "materias"));
  // Clave simple
  EXPECT_TRUE(db.registroValido( Registro({"LU", "Nombre", "Editor", "OS"}, {datoStr("2/90"), datoStr("March"), datoStr("Vim"), datoStr("Linux")}), "alumnos"));
}
